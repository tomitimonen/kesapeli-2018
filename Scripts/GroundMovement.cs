...
public class GroundMovement : MonoBehaviour {
    ...
    public float jumpForce = 333.333f;
    ...
    public bool jump(float power)
    {
        if (canJump && onGround)
        {
            rb.AddForce(0, jumpForce * power, 0, ForceMode.Impulse);
            canJump = false;
            //Debug.Log("GroundMovement - Jump");
        }
        return canJump;
    }
	...
	void Update () {
        ...
        if (showDebug) Debug.DrawLine(transform.position + feetPosition, transform.position + feetPosition + Vector3.down * jumpRaycastDistance, Color.red);

        RaycastHit hit;

        if (Physics.Raycast(transform.position + feetPosition, Vector3.down, out hit, jumpRaycastDistance)
            && hit.collider.gameObject.CompareTag("LevelCollision"))
        {
            if (!onGround)
            {
                ...
                onGround = true;
                timeOnAir = 0;
                if (OnHitGround != null) OnHitGround();
            }
        }
        else
        {
        ...
        //if (showDebug) Debug.Log("GroundMovement - onGround: " + onGround);
        ...
        raycastWallForward |= raycastLevelCollision(startPoint, invertDir * transform.forward, rayDistance);
        //keskisäteet
        raycastWallBackward |= raycastLevelCollision(transform.position, -invertDir * transform.forward, rayDistance);
        raycastWallForward |= raycastLevelCollision(transform.position, invertDir * transform.forward, rayDistance);
    ...
    private void FixedUpdate()
    {
        ...
        {
            float currentForce = onGround ? runForce : jumpForceMultiplier * runForce;
            Vector3 runForceAdd = keeper.outHorTangent * currentForce * relativeDT;
        ...
        //nopeampi putoaminen hypyn huippukohdasta
        if (!onGround)
        {
            if (rb.velocity.y < 0)
            {
